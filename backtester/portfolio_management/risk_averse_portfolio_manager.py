...
class RiskAversePortfolioManager(PortfolioManagerInterface):
    """Portfolio manager with conservative stake allocation and strict deactivation."""
    
    def __init__(self, initial_bankroll: float = 1000.0, max_stake_fraction: float = 0.05):
        """Initialize the risk-averse portfolio manager.
        
        Args:
            initial_bankroll (float): Starting capital (default: 1000.0).
            max_stake_fraction (float): Maximum bankroll fraction per bet (default: 0.05).
        """
        super().__init__()
    ...
    def add_strategy(self, strategy_name: str) -> None:
        """Add a new strategy with conservative initial metrics."""
    ...
    def update_strategy(self, strategy_name: str, pnl: float) -> None:
        """Update the strategy with conservative risk controls."""
        ...
        # Stricter deactivation: 3 loss streak or 20% drawdown
        if p['loss_streak'] >= 3 or (drawdown / p['peak'] > 0.2 if p['peak'] > 0 else False):
            p['active'] = False
    ...
    def get_strategy_metrics(self, strategy_name: str) -> Dict[str, Any]:
        """Retrieve conservative strategy metrics."""
        if strategy_name not in self.portfolios:
            return {}
    ...
    def get_overall_metrics(self) -> Dict[str, Any]:
        """Retrieve overall conservative portfolio metrics."""
