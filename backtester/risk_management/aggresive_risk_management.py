...
class AggressiveRiskManager(RiskManagerInterface):
    """Aggressive risk manager for sports betting strategies."""
    
    def __init__(self):
        """Initialize the aggressive risk manager with default configuration."""
        super().__init__()
        self.strategy_risk = {}
        self.default_config = {
            'max_loss_streak': 10,  # Very lenient
            'max_drawdown_pct': 0.5,  # 50% drawdown
            'max_exposure': 0.3,  # 30% of bankroll
            'dynamic_stake_pct': 0.1,  # 10% of bankroll
            'min_bankroll': 100  # Low threshold
        }
